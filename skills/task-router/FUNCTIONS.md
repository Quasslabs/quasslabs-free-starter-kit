...
| Item | Value | Notes |
|---|---|---|
| Bot/agents | operator · any | Pre-flight evaluator. Runs as Step 0 before any skill execution. Returns a routing card that assigns each sub-task to the right AI model, identifies human gates (red = immediate blocker, yellow = note and continue), and estimates cost. Prevents mid-task stalls caused by missing credentials or unclear requirements. |
...
| Extract deterministic helpers before calling AI for task-router. |
